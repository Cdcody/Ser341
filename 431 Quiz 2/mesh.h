...
Mesh* createTerrain(int precision, double** points) {
	...
	for (int i = 0; i < precision; i++) {
		for (int j = 0; j < precision; j++) {
			mesh->dot_vertex.push_back(Vec3<GLfloat>(i, points[i][j], j));//create vertices with the x and z coordinates + derived y
		}
	}
	...
	for (int i = 0; i < precision - 1; i++) {
		for (int j = 0; j < precision - 1; j++) {
			...
			int index = precision * i + j;
			mesh->face_index_vertex.push_back(index);//current point
			mesh->face_index_vertex.push_back(index + precision);//same point n next row
			mesh->face_index_vertex.push_back(index + precision + 1);//next row, 1 point down

			mesh->face_index_vertex.push_back(index);//current point
			mesh->face_index_vertex.push_back(index + precision + 1);//next point
			mesh->face_index_vertex.push_back(index + 1);//next row, 1 point down
		}
	}
